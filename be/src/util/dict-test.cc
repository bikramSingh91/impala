...
void ValidateDict(const vector<LOGICAL_TYPE>& values,
    const vector<LOGICAL_TYPE>& dict_values, int fixed_buffer_byte_size) {
  set<LOGICAL_TYPE> values_set(values.begin(), values.end());

  MemTracker tracker;
  MemPool pool(&tracker);
  DictEncoder<LOGICAL_TYPE> encoder(&pool, fixed_buffer_byte_size);
  for (LOGICAL_TYPE i: values) encoder.Put(i);
  ...
  DictDecoder<LOGICAL_TYPE, PHYSICAL_TYPE> decoder;
  ASSERT_TRUE(
  ...
  for (int i = 0; i < dict_values.size(); ++i) {
    LOGICAL_TYPE expected_value = dict_values[i];
    LOGICAL_TYPE out_value;
...
void IncrementValue(LOGICAL_TYPE* t) { ++(*t); }
...
void TestNumbers(int max_value, int repeat, int value_byte_size) {
  vector<LOGICAL_TYPE> values;
  vector<LOGICAL_TYPE> dict_values;
  for (LOGICAL_TYPE val = 0; val < max_value; IncrementValue(&val)) {
  ...
  ValidateDict<LOGICAL_TYPE, PHYSICAL_TYPE>(values, dict_values, value_byte_size);
}

template<typename LOGICAL_TYPE, parquet::Type::type PHYSICAL_TYPE>
void TestNumbers(int value_byte_size) {
  TestNumbers<LOGICAL_TYPE, PHYSICAL_TYPE>(100, 1, value_byte_size);
  TestNumbers<LOGICAL_TYPE, PHYSICAL_TYPE>(1, 100, value_byte_size);
  TestNumbers<LOGICAL_TYPE, PHYSICAL_TYPE>(1, 1, value_byte_size);
  TestNumbers<LOGICAL_TYPE, PHYSICAL_TYPE>(1, 2, value_byte_size);
}
...
TEST(DictTest, TestInvalidStrings) {
  ...
  DictDecoder<StringValue, parquet::Type::BYTE_ARRAY> decoder;
